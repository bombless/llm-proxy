...
<template>
  <section class="card usage-panel">
    <div class="section-head">
      ...
      <div class="usage-total">累计 {{ money(total) }} <button class="btn secondary" :disabled="loading" @click="load">{{ loading ? '刷新中…' : '刷新' }}</button></div>
    </div>
    <div v-if="!rows.length" class="empty">暂无配置。</div>
    <div v-for="row in rows" :key="`${row.type.key}:${row.config.id}`" class="usage-config">
      <div class="usage-config-head"><div><strong>{{ row.config.public_model }}</strong><span class="metric-type">{{ row.type.title }}</span></div><strong>{{ money(row.item?.total_cost) }}</strong></div>
      <div class="usage-grid">
        ...
        <div><span>缓存费用</span><strong>{{ money(row.item?.cache_cost) }}</strong></div>
        <div><span>预填充费用</span><strong>{{ money(row.item?.prefill_cost) }}</strong></div>
        <div><span>生成费用</span><strong>{{ money(row.item?.generation_cost) }}</strong></div>
      </div>
    </div>
  </section>
</template>
